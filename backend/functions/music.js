...
export default {
  createPlaylist: (userId, details) => {
    return new Promise(async (resolve, reject) => {
      let response;
      try {
        response = await database.collection(collections.LIBRARY).insertOne({
  ...
  deletePlaylist: (userId, details) => {
    return new Promise(async (resolve, reject) => {
      try {
        let response = await database.collection(collections.LIBRARY).updateOne(
          {
            _id: new ObjectId(userId),
          },
          {
  ...
  checkInLibrary: (userId, id, type) => {
    return new Promise(async (resolve, reject) => {
      try {
        let response = await database.collection(collections.LIBRARY).findOne({
          _id: new ObjectId(userId),
  ...
  getUserPlaylist: (userId, playlistId) => {
    return new Promise(async (resolve, reject) => {
      try {
        let response = {};
        response.details = await database
          .collection(collections.LIBRARY)
          .aggregate([
            {
              $match: {
        ...
        response.tracks = await database
          .collection(collections.LIBRARY)
          .aggregate([
            {
              $match: {
        ...
        if (response) {
          if (response?.details?.[0]) {
            ...
            reject("Playlist not found on SonicWave");
          }
        }
      } catch (err) {
        reject(err);
  ...
  recentActivity: (userId, artist) => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (total?.[0]?.value >= 10) {
          let remove = await database.collection(collections.ACTIVITY).updateOne(
            {
              _id: new ObjectId(userId),
            },
            {
          ...
          if (remove) {
            await database.collection(collections.ACTIVITY).updateOne(
              {
                _id: new ObjectId(userId),
              },
              {
  ...
  clearHistory: (userId) => {
    return new Promise(async (resolve, reject) => {
      try {
        let res = await database.collection(collections.LIBRARY).updateOne(
          {
            _id: new ObjectId(userId),
          },
          {
