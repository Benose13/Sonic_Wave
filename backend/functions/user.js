...
export default {
  register_request: (details) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        let check = await database.collection(collections.USER).findOne({
        ...
        if (!check) {
          ...
          response = await database.collection(collections.TEMP).insertOne({
            ...details,
            expireAt: new Date(),
          });
        } else {
          reject({ status: 422, message: "Registered Already" });
        }
      } catch (err) {
        if (err?.code === 11000) {
  ...
  register_complete: (id, secret) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        let already = await database.collection(collections.USER).findOne({
          _id: new ObjectId(id),
        });

        if (!already) {
          let temp = await database.collection(collections.TEMP).findOne({
            _id: new ObjectId(id),
            secret,
          });
          ...
          if (temp) {
            ...
            response = await database.collection(collections.USER).insertOne(temp);
          } else {
  ...
  register_direct: (details) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        response = await database.collection(collections.USER).insertOne(details);
      } catch (err) {
        if (err?.code === 11000) {
          reject({ status: 422, message: "Registered Already" });
        } else {
  ...
  login_manual: (email, password) => {
    return new Promise(async (resolve, reject) => {
      try {
        let user = await database.collection(collections.USER).findOne({
          email,
        });
  ...
  forgot_complete: (id, secret) => {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (temp) {
          response = await database.collection(collections.USER).updateOne(
            {
              _id: new ObjectId(id),
            },
            {
