...
router.post("/register", CheckLogged, async (req, res) => {
  ...
  if (password?.length >= 8 && password === rePassword) {
    if (google) {
      ...
      if (email?.match(validRegex)) {
        ...
        try {
          ...
          if (response?._id) {
            fs.readFile(
              `${path.resolve(`${path.dirname("")}/mail/mailbox/index.html`)}`,
              "utf-8",
              (err, html) => {
                if (!err && html) {
                  ...
                  sendMail(
                    {
                      to: email,
                      subject: `SonicWave Register Verification`,
                      html,
                    },
                    (err, done) => {
                      if (err) {
    ...
    res.status(422).json({
      status: 422,
      message:
        "Password length must contain 8 digits and password and Re Enter password must same",
    });
  }
});
            ...
            (err, html) => {
              if (!err && html) {
                ...
                sendMail(
                  {
                    to: email,
                    subject: `SonicWave Forgot Password Verification`,
                    html,
                  },
                  (err, done) => {
                    if (err) {
...
router.put("/edit-profile", async (req, res) => {
  ...
  if (password_profile?.length >= 8) {
    ...
    res.status(422).json({
      status: 422,
      message: "Password Length Must 8 Digits",
    });
  }
});
