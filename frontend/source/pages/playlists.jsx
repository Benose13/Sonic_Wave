...
import { LibraryHead, LibraryModal, LoadMore, Row } from "../plugins";
...
const Playlists = () => {
  ...
  const loadMore = async (offset) => {
    ...
    try {
      ...
      if (err?.response?.data?.status === 405) {
        ...
        alert("Facing An Error(Playlists)");
        return true;
      }
    } finally {
      if (res?.data) {
    ...
    if (type === "edit") {
      ...
      try {
        ...
        if (err?.response?.data?.status === 405) {
          ...
          alert("Facing An Error(Edit Playlist)");
        }
      } finally {
        if (res?.data) {
          setState((state) => ({
      ...
      try {
        ...
        if (err?.response?.data?.status === 405) {
          ...
          alert("Facing An Error(Create Playlist)");
        }
      } finally {
        if (res?.data) {
      ...
      try {
        ...
        if (err?.response?.data?.status === 405) {
          ...
          alert("Facing An Error(Delete Playlist)");
        }
      } finally {
        if (res?.data) {
          setState((state) => ({
  ...
  const getPlaylists = async (cancelToken, search) => {
    ...
    try {
      ...
      if (axios.isCancel(err)) {
        ...
        alert("Facing An Error(Playlist)");
        setTimeout(() => {
  ...
  useEffect(() => {
    document.title = `SonicWave - Library`;
