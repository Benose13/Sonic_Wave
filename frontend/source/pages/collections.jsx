...
const Collections = ({ isArtist }) => {
  ...
  const loadMoreTracks = async () => {
    ...
    try {
      ...
      if (typeof err?.response?.data?.message === "string") {
        ...
        alert("Facing An Error(Album-tracks-more)");
      }
    ...
    if (checked) {
      ...
      try {
        ...
        if (err?.response?.data?.status === 405) {
          ...
          alert("Facing An Error(Add Track)");
        }
      } finally {
        if (res?.data) {
      ...
      try {
        ...
        if (err?.response?.data?.status === 405) {
          ...
          alert("Facing An Error(Remove Track)");
        }
      } finally {
        if (res?.data) {
          reloadData(search?.value);
  ...
  useEffect(() => {
    document.title = `SonicWave - ${isArtist ? "Artist" : "Album"}`;
