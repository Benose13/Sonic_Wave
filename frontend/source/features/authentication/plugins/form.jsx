...
import { Google } from "../../../accessories";
    ...
    if (isSignup) {
      if (state?.form?.password?.length >= 8) {
        if (state?.form?.password === state?.form?.rePassword) {
          ...
          errorHandle("Password And Re-Password Must Be Equal");
        }
      } else {
        errorHandle("Enter Password And Password Length Must Contain 8 Digits");
      }
    } else {
      if (state?.form?.password?.length >= 8) {
        login();
